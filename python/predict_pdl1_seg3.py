...
def compute_seg3_boundary(cell_mat, view=False, n_neighbors=15, slide_id='xxx', remove_blank_regions=True, recompute=False):
    ...
    try:
        ...
        Z = np.load(slide_id.split(".npy")[0] + "_seg3.npy")

        print "Precomputed seg3 decision boundary found."

    except IOError:

        tumor_loc = np.nonzero(cell_mat == 1)
        x_tumor = np.vstack((tumor_loc[1], tumor_loc[0])).T

        nontumor_loc = np.nonzero(cell_mat > 2)
        x_nontumor = np.vstack((nontumor_loc[1], nontumor_loc[0])).T

        pdl1_loc = np.nonzero(cell_mat == 2)
        x_pdl1 = np.vstack((pdl1_loc[1], pdl1_loc[0])).T

        X = np.vstack((x_nontumor, x_tumor, x_pdl1))
        y = np.array(x_nontumor.shape[0] * [0] + x_tumor.shape[0] * [1] + x_pdl1.shape[0] * [2])
        ...
        np.save(slide_id.split(".npy")[0] + "_seg3.npy", Z)
...
def extract_features_around_points(ii, jj, R, decision_boundary, cell_mat, Rmin=0, flag=False):
    ...
    for pt in range(len(ii)):
        ...
        density_foxp3 = n_foxp3 / n_stromal_cells
        density_cd8 = n_cd8 / n_stromal_cells
        density_cd4 = n_cd4 / n_stromal_cells
        density_pdmac = n_pdmac / n_stromal_cells
        density_other = n_other / n_stromal_cells
        density_tumor = n_tumor / n_stromal_cells
        density_pdl1 = n_pdl1 / n_stromal_cells
...
if __name__ == '__main__':
    ...
    SAMPLES = [all_samples[i] for i in np.random.choice(len(all_samples), 50)]
    ...
    for slide in SAMPLES:
        ...
        db = compute_seg3_boundary(cell_mat, view=True, n_neighbors=25, slide_id=slide, recompute=False, remove_blank_regions=True)
        ...
        # db = ndimage.median_filter(db, size=30)     # first clean up boundary image
        ...
        R_vec = [50, 100, 150, 250]
        ivec, jvec, identity, circles = sample_circles(decision_boundary=db, cell_mat=cell_mat, r=R_vec[0],
        ...
        features0 = extract_features_around_points(ii=ivec, jj=jvec, R=R_vec[0], decision_boundary=db,
                                                  cell_mat=cell_mat, Rmin=0, flag=True)
        features1 = extract_features_around_points(ii=ivec, jj=jvec, R=R_vec[1], decision_boundary=db,
                                                  cell_mat=cell_mat, Rmin=R_vec[0], flag=False)
        features2 = extract_features_around_points(ii=ivec, jj=jvec, R=R_vec[2], decision_boundary=db,
                                                  cell_mat=cell_mat, Rmin=R_vec[1], flag=False)

        # features3 = extract_features_around_points(ii=ivec, jj=jvec, R=75, decision_boundary=db,
        #                                           cell_mat=cell_mat, Rmin=50)
        # features4 = extract_features_around_points(ii=ivec, jj=jvec, R=100, decision_boundary=db,
        #                                           cell_mat=cell_mat, Rmin=75)
        features5 = extract_features_around_points(ii=ivec, jj=jvec, R=R_vec[3], decision_boundary=db,
                                                  cell_mat=cell_mat, Rmin=R_vec[2], flag=False)
        # features3 = extract_features_around_points(xs=ii, ys=jj, R=100, decision_boundary=db,
        #                                           cell_mat=cell_mat, Rmin=70)

        # features = features1
        features = np.hstack((features0, features1, features2, features5))
    ...
    combined_response[tmp <= 0.0] = 0
    combined_response[(tmp > 0) & (tmp <= 0.5)] = 1
    combined_response[tmp > 0.5] = 2
