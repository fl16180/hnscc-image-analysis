...
for sample in SAMPLES:
    ...
    # load slide
    cells = load_sample(DIR + sample, verbose=False, radius_lim=200)

    # sample tumor cells from slide
    tumor_sample, response = get_random_sample(cells=cells['all_tumor'], n_cells=30)
    ...
    features6 = get_binned_distances(tumor_sample, 'pdmac', max_radius=200, n_bins=50)
    features7 = get_binned_distances(tumor_sample, 'other', max_radius=200, n_bins=50)


    features = np.hstack((features3, features4, features5, features6, features7))
    # features = features7


    combined_sample.append(features)
    combined_response.extend(response)
    combined_nn_pred.extend(nn_pred)

combined_sample = np.vstack(combined_sample)
combined_response = np.array(combined_response)
combined_nn_pred = np.array(combined_nn_pred)

print combined_sample.shape
print combined_response.shape
print combined_nn_pred.shape
...
for i in range(n_iter):
    ...
    two_class_score(combined_nn_pred[id_test], y_test, label='1-NN')
